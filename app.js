...
async function handleLogin() {
    ...
    if (error) document.getElementById('login-error').innerText = "Feil e-post eller passord.";
...
async function changePassword() {
    ...
    if (ny && ny.length >= 6) {
        ...
        alert(error ? error.message : "Oppdatert!");
    }
}
...
function buildTimeline() {
    ...
    for (let i = 0; i < 64; i++) {
        ...
        data.timeline.push({ id, week: info.week, month: monthNames[info.month], year: info.year, isPast: d < currentMonday && id !== currentWeekId });
    }
}
...
function scrollToCurrentWeek(instant) {
    ...
    if (container && currentWeekEl) {
        ...
        if (instant) container.scrollLeft = pos;
        else container.scrollTo({ left: pos, behavior: 'smooth' });
...
function syncTopFromMain() {
    ...
    if (scrollTop && scrollMain) setScrollRatio(scrollTop, getScrollRatio(scrollMain));
}
...
// ============================================================
// YTELSE: Pre-indeksering av assignments
// Bygger en serie Maps med ferdig-summerte verdier per (ansatt|uke),
// (prosjekt|uke) osv. Erstatter tusenvis av .filter()-kall i renderUI
// med raske Map-oppslag.
// ============================================================
function buildAssignmentIndex() {
    ...
    data.assignments.forEach(a => {
        ...
        if (pros > 0) {
            ...
            const m = empProj.get(aId);
            if (!m.has(pId)) m.set(pId, new Set());
            m.get(pId).add(uke);

            if (!projEmp.has(pId)) projEmp.set(pId, new Map());
            const m2 = projEmp.get(pId);
            if (!m2.has(aId)) m2.set(aId, new Set());
            m2.get(aId).add(uke);
        }
    });
...
async function exportProsjektliste() {
    ...
    const buildRows = (prosjekter) => {
        return prosjekter.map(p => {
            const row = [p.prosjektnummer || '', p.navn, p.er_nc ? 'NC' : (p.er_ufakturerbart ? 'Ufakt.' : 'Nordic')];
            tre.forEach(m => {
...
async function exportToExcel() {
    ...
    ansatte.forEach(ansatt => {
        ...
        blokk.forEach(r => finalRows.push([r.avdeling, r.klynge, r.ansattNavn, r.prosjektNavn, ...uker.map(u => r[u] ?? '')]));
    ...
    wsPivot['!cols'] = [{ wch: 15 }, { wch: 15 }, { wch: 20 }, { wch: 40 }, ...uker.map(() => ({ wch: 7 }))];
...
async function importFromExcel(event) {
    if (currentUserRole !== 'superbruker') return alert("Kun superbrukere kan gjenopprette fra backup.");
...
async function importAnsatteFromExcel(event) {
    if (currentUserRole !== 'superbruker') return alert("Kun superbrukere kan importere ansatter.");
    const file = event.target.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = async (e) => {
        ...
        json.forEach(row => {
            ...
            if (!navn) return;
            if (eksisterende.includes(navn.toLowerCase())) { duplikater.push(navn); return; }
            nyeAnsatte.push({
                navn,
                ...
                rolle: ['admin', 'ansatt', 'superbruker'].includes(String(row[headerMap['rolle']] || '').toLowerCase())
                    ? String(row[headerMap['rolle']]).toLowerCase() : 'ansatt'
            });
        });
...
function renderUI() {
    ...
    // YTELSE: Bygg index én gang per render i stedet for tusenvis av .filter()-kall
    ...
    if (view === 'ansatte' || view === 'ledig') {
        getFilteredEmployees().forEach(emp => {
            ...
            if (expanded.has(emp.id)) {
                ...
                alleProsjekter.filter(pId => {
                    const ukeSet = empProsjMap.get(String(pId));
                    if (ukeSet) for (const u of ukeSet) if (sjekkSet.has(u)) return true;
                    ...
                    data.timeline.forEach(t => {
                        ...
                        if ((t.isPast && t.id !== currentWeekId) || !canEditOwnRow) {
                            ...
                            ph += `<td class="${isU ? 'cell-unsure' : ''}" title="Høyreklikk for meny"><input class="cell-input" value="${formatCellValue(v, isU)}" data-ansatt-id="${esc(emp.id)}" data-prosjekt-id="${esc(pId)}" data-uke="${esc(t.id)}" data-action="cell-input"></td>`;
                        }
                    });
                    ptr.innerHTML = ph;
        ...
        sorterteProsjekter.forEach(p => {
            ...
            data.timeline.forEach(t => {
                ...
                h += `<td id="${safeId('sum_proj', p.id, t.id)}" style="background-color:${getCellColor(s)}; color:${s > 0 ? 'inherit' : 'transparent'}">${s > 0 ? s + '%' : '-'}</td>`;
            });
            tr.innerHTML = h;
            body.appendChild(tr);
            ...
            if (expanded.has(p.id)) {
                ...
                alleAnsatte.filter(aId => {
                    ...
                    data.timeline.forEach(t => {
                        ...
                        const v = a ? a.prosent : 0;
                        const isU = a ? a.er_usikker : false;
                        if ((t.isPast && t.id !== currentWeekId) || !canEditRow) {
                            ph += `<td class="cell-locked">${formatCellValue(v, isU)}</td>`;
                        } else {
                            ph += `<td class="${isU ? 'cell-unsure' : ''}"><input class="cell-input" value="${formatCellValue(v, isU)}" data-ansatt-id="${esc(aId)}" data-prosjekt-id="${esc(p.id)}" data-uke="${esc(t.id)}" data-action="cell-input"></td>`;
                        }
                    });
                    ptr.innerHTML = ph;
                    body.appendChild(ptr);
...
function initListeners() {
    ...
    // ── FIKSET: én ren versjon av handleCellUpdate uten duplikater ──
    const handleCellUpdate = (inp) => {
        ...
        // Lagre original-verdi kun hvis ikke allerede en pending save
        if (!pendingSaves[id]) {
            pendingSaves[id] = {
                ansattId: aId, prosjektId: pId, uke, value: inp.value,
                origValue: ex ? ex.prosent : 0,
                origUsikker: ex ? ex.er_usikker : false
            };
        } else {
        ...
        // Oppdater visuelt (kun cellen + total)
        oppdaterLokalSum(aId, pId, uke);

        // Debounce save til database
        ...
        saveTimeouts[id] = setTimeout(async () => {
            ...
            if (orig && !_batchUndoActive) {
                pushUndo([{
                    ...
                    oldValue: orig.origValue, oldUsikker: orig.origUsikker
                }]);
            }
    ...
    container.addEventListener('mousedown', (e) => {
        const inp = e.target.closest('input[data-action="cell-input"]');
        if (!inp) return;
        if (e.shiftKey) { e.preventDefault(); addCellToSelection(inp); return; }
        isDragging = true;
        dragStartCell = inp;
        clearSelection();
    ...
    window.addEventListener('mouseup', () => { isDragging = false; dragStartCell = null; });

    document.addEventListener('mousedown', (e) => {
        if (!e.target.closest('#mainTableContainer') && selectedCells.size > 0) clearSelection();
    });
    ...
    function handleArrowNavigation(e) {
        ...
        if (e.key === 'ArrowRight') {
            ...
            if (targetRow && targetRow.children[cellIndex]) targetInput = targetRow.children[cellIndex].querySelector('input, select');
...
function updateChart() {
    ...
    if (view === 'prosjekter') {
        ...
        if (projsUtenAvd.length > 0) {
            ...
            if (dataPoints.some(v => v > 0)) {
                datasets.push({ label: 'Fakturerbar (Uten avd)', data: dataPoints, borderColor: '#a1a1aa', backgroundColor: '#a1a1aa80', fill: true, tension: 0.1, borderWidth: 1, pointRadius: 0 });
            }
        }
        ...
        const totalUtil = chartTimeline.map(t => {
            let s = 0;
            fEmps.forEach(e => { s += idx.empSumSure.get(`${e.id}|${t.id}`) || 0; });
            return Math.round(s / n);
        });
        const unsureUtil = chartTimeline.map(t => {
            let s = 0;
            fEmps.forEach(e => { s += idx.empSum.get(`${e.id}|${t.id}`) || 0; });
            return Math.round(s / n);
        });
        ...
        if (view === 'ledig') {
            ...
            const faktUtil = chartTimeline.map(t => {
                let s = 0;
                fEmps.forEach(e => { s += idx.empSumFakt.get(`${e.id}|${t.id}`) || 0; });
                return Math.round(s / n);
            });
    ...
    const scaleOptions = { y: { beginAtZero: true, stacked: isStacked, ticks: { color: tickColor } }, x: { stacked: isStacked, ticks: { color: tickColor } } };
    if (yMax !== undefined) scaleOptions.y.max = yMax;

    chartInstance = new Chart(ctx, {
        type: 'line', data: { labels, datasets },
        options: { responsive: true, maintainAspectRatio: false, scales: scaleOptions, plugins: { annotation: { annotations }, legend: { labels: { color: tickColor } }, tooltip: { mode: isStacked ? 'index' : 'nearest', intersect: false } } }
    });
}
...
async function saveProject() {
    ...
    const payload = {
        prosjektnummer: nr, navn,
        avdeling: avdeling || null, gruppe: gruppe || null,
        er_nc: type === 'nc', er_ufakturerbart: type === 'uf', arkivert
    };
...
async function executeMerge() {
    ...
    oldAssignments.forEach(oldA => {
        const existing = newAssignments.find(nA => String(nA.ansatt_id) === String(oldA.ansatt_id) && nA.uke === oldA.uke);
        if (existing) {
            toUpsert.push({
                id: existing.id, ansatt_id: existing.ansatt_id, prosjekt_id: newId, uke: existing.uke,
            ...
            toUpsert.push({
                ...
                ansatt_id: oldA.ansatt_id, prosjekt_id: newId, uke: oldA.uke,
                prosent: Number(oldA.prosent) || 0, er_usikker: !!oldA.er_usikker
            });
        }
    });
    ...
    if (toUpsert.length > 0) {
        for (let i = 0; i < toUpsert.length; i += 1000) {
            const { error: upErr } = await db.from('bemanning').upsert(toUpsert.slice(i, i + 1000), { onConflict: 'id' });
            if (upErr) {
                document.getElementById('saveStatus').innerText = "";
...
async function saveEmployee() {
    ...
    if (id) await db.from('ansatte').update({ navn, avdeling, gruppe: klynge, email: epost, rolle }).eq('id', id);
    else {
...
async function flushPendingSaves() {
    ...
    pending.forEach(p => {
        ...
        const body = JSON.stringify({
            id, ansatt_id: p.ansattId, prosjekt_id: p.prosjektId, uke: p.uke,
            prosent: val, er_usikker: isUnsure
        });
        try {
...
// Synkroniser i bakgrunnen hvert 5. minutt
setInterval(async () => {
